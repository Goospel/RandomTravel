...
/**
 * 대상의 뷰포트 기준 top(getBoundingClientRect().top). 위로 벗어났으면 머리부터(start) —
 * nearest 는 "위로 벗어남 & 뷰포트보다 큼"을 아래 정렬로 풀어 제목을 못 보여준다
 * ("근처에서 한 번 더"를 카드 하단에서 눌렀을 때). 나머지는 nearest(이미 보이면 no-op).
 */
export function revealBlock(topInViewport: number): ScrollLogicalPosition {
  return topInViewport < 0 ? "start" : "nearest";
}
