...
- **changeLog.md** — 완료 기록(역순 · 왜/무엇만, 2026-07-28 신설). 자체 PR 번호는 안 적는다(스쿼시 커밋 제목이 단일 출처).
- **designGuide.md** — 디자인 규칙 단일 출처(색·타이포·모양·모션·톤). **UI를 만들거나 고치기 전에 반드시 읽는다** — 여기 없는 색·크기를 즉흥으로 쓰지 않는다.
...
- **마일스톤 완료 시 4종 갱신**: plan.md(§ 소절 + 마일스톤 표 행) · README 체크리스트 · changeLog.md 항목 · 자동 메모리.
